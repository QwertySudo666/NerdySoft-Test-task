...
//        var isUsed = false;
        ...
        System.out.println("Is xxx code used: " + isCodeUsed + "\n");
        isCodeUsed = VirtualProductCodeManager.isCodeUsed("yyy");
        System.out.println("Is yyy code used: " + isCodeUsed + "\n");
//        System.out.println("Is code used: " + isUsed + "\n");
    ...
    private static Product getMostPopularProduct(List<Order> orders) {
        Map<Product, List<User>> productUsersMap = orders.stream()
                .flatMap(order -> order.getProducts().stream()
                        .map(product -> new AbstractMap.SimpleEntry<>(product, order.getUser())))
                .collect(Collectors.groupingBy(AbstractMap.SimpleEntry::getKey,
                        Collectors.mapping(AbstractMap.SimpleEntry::getValue, Collectors.toList())));
        Optional<Map.Entry<Product, List<User>>> productEntry = productUsersMap.entrySet().stream()
                .max(Comparator.comparingInt(entry -> entry.getValue().size()));
        return productEntry.get().getKey();
    }

    private static double calculateAverageAge(Product product, List<Order> orders) {
        List<Order> filteredOrders = orders.stream().filter(order -> order.getProducts().contains(product)).toList();
...
//        List<Order> sortedOrders = orders.stream().sorted(Comparator.comparingInt(order -> order.getUser().getAge())).toList();
    ...
    private static Map<Order, Integer> calculateWeightOfEachOrder(List<Order> orders) {
        return orders.stream().collect(Collectors.toMap(
                ...
                order -> order.getProducts().stream()
                        .filter(it -> it.getClass() == Real.class)
                        .map(real -> (Real) real)
                        .mapToInt(real -> (int) real.getWeight())
                        .sum()
        ));
    }
}